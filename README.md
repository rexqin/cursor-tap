...
# 或分别启动
nx dev tap      # Go 代理（:8080 / :1080 / :9090）
nx dev web      # Next.js WebUI（:3000）

# 其他常用命令
nx build tap    # 构建 Go 二进制 → dist/tap
nx build web    # 构建前端
nx run proto:generate  # buf generate
nx test tap     # go test ./...
```
...
```
├── cmd/cursor-tap/     # 代理入口（Nx: tap）
├── internal/
│   ├── ca/             # CA 证书管理，动态签发
│   ├── proxy/          # HTTP CONNECT 代理
│   └── httpstream/     # gRPC 解析核心
├── cursor_proto/       # proto 定义（Nx: proto）
