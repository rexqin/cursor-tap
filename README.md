...
- [项目架构](./docs/architecture/index.md) — monorepo 结构、模块职责、数据流与 API
...
# 其他常用命令
pnpm build:tap  # 构建 Go 代理 → dist/tap
pnpm build:api  # 构建 API 服务 → dist/tap-api
pnpm build:web  # 构建前端
...
```bash
# 终端 1：API 服务（REST + WebSocket）
go run ./apps/api start

# 终端 2：MITM 代理（写入 SQLite 并通知 API）
go run ./apps/tap start --http-parse
```

代理默认监听 `localhost:8080`（HTTP）和 `localhost:1080`（SOCKS5）。API 默认监听 `localhost:9090`。
...
详见 [项目架构](./docs/architecture/index.md)。
